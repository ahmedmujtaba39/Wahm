...
Computes four automated metrics per translation:
  ...
  3. Back-translation similarity (token-F1 of round-tripped MSA vs original)
  4. Dialect distance (token-F1 of dialect output vs MSA original, lower = more dialectal)

Reads   candidates_<dialect>.csv
Writes  scores_translation_<dialect>.csv  (per-row scores)
        scores_translation_<dialect>_summary.json  (aggregate stats)

Usage:
    pip install bert-score  # needs torch
    python score_translation.py gulf
"""
...
def run(dialect):
    ...
    # 1. Back-translation similarity
    bt_sims = [token_f1(src, bt) if bt else None
               for src, bt in zip(sources, backtranslations)]

    # 2. Dialect distance (how far from MSA)
    ...
    out = f"scores_translation_{dialect}.csv"
    with open(out, "w", encoding="utf-8", newline="") as f:
        w = csv.writer(f)
        w.writerow(["qid", "question_msa", "dialect_candidate",
                    "backtrans_similarity", "dialect_distance",
    ...
    summary = {
        ...
        "backtrans_similarity_mean": safe_mean(bt_sims),
        "dialect_distance_mean": safe_mean(dd_sims),
    ...
    summary_path = f"scores_translation_{dialect}_summary.json"
