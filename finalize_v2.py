...
Reads   candidates_<dialect>.csv    (machine output + back-translations)
        validation_<dialect>_v2.csv (returned v2 validator sheet)
Writes  final_<dialect>_v2.csv      (the v2 benchmark arm)
        metrics_<dialect>_v2.json   (the v2 paper-table metrics)
...
Metrics reported per dialect
    ...
    backtrans_cosine_similarity  embedding meaning similarity  (validity)
    dialect_lexical_overlap      did it actually leave MSA?     (authenticity)
    mean_fluency           validator 1-5 naturalness rating    (AAVENUE-style)
    filtered_out           rows failing the hard filter

Hard filter
    A row whose back-translation cosine is below --min-backtrans AND which
...
Usage
    python finalize_v2.py gulf
    python finalize_v2.py gulf --min-backtrans 0.45
"""
...
def load(dialect):
    ...
    try:
        with open(f"validation_{dialect}_v2.csv", encoding="utf-8", newline="") as source:
            val = list(csv.DictReader(source))
    except FileNotFoundError:
        sys.exit(f"ERROR: validation_{dialect}_v2.csv not found. "
...
def run(dialect, min_backtrans=0.4):
    ...
    candidate_rows = [cand.get(v["qid"]) for v in val]
    bt_scores = backtrans_cosine_similarities(
        [c["question_msa"] if c else "" for c in candidate_rows],
        [c.get("backtranslation_msa", "") if c else "" for c in candidate_rows],
    )

    rows, unreviewed = [], 0
    for v, bt in zip(val, bt_scores):
        qid = v["qid"]
        c = cand.get(qid)
        if not c:
            continue
        ...
        # Keep lexical overlap for dialect divergence separate from semantic
        # cosine similarity of the MSA round trip.
        ...
        rows.append(dict(qid=qid, msa=c["question_msa"], gold=c["gold_answer"],
                         ...
                         backtrans_cosine_similarity=bt,
                         dialect_lexical_overlap=dd, edit=ed,
    ...
    bts = [r["backtrans_cosine_similarity"] for r in rows
           if r["backtrans_cosine_similarity"] is not None]
    ...
    metrics = {
        ...
        "mean_backtrans_cosine_similarity": round(statistics.mean(bts), 3) if bts else None,
        "mean_dialect_lexical_overlap": round(
            statistics.mean(r["dialect_lexical_overlap"] for r in kept), 3) if kept else None,
        ...
        "min_backtrans_cosine_threshold": min_backtrans,
    }

    out = f"final_{dialect}_v2.csv"
    ...
    with open(f"metrics_{dialect}_v2.json", "w", encoding="utf-8") as f:
    ...
    print(f"wrote metrics_{dialect}_v2.json")

    # Authenticity warning: high lexical overlap means it barely left MSA.
    dd = metrics["mean_dialect_lexical_overlap"]
    if dd and dd > 0.85:
        print(f"\n  WARNING: mean dialect lexical overlap {dd} is very high — this arm "
